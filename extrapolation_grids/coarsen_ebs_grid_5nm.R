...
# original resolution is 2x2nm (3704, 3704), coarsened to 10x10nm here
ebs_grid <- akgfmaps:::make_2d_grid(obj = dplyr::filter(ebs_layers$survey.area, 
                                                        SURVEY_DEFINITION_ID == 98),
                                    resolution = c(3704, 3704)*5, 
                                    bbox = bbox,
                                    output_type = "point",
...
write.csv(ebs_grid, row.names = FALSE,
          file = here::here("extrapolation_grids", "ebs_coarse_grid_5nm.csv"))

# NBS alone ----
nbs_grid <- akgfmaps:::make_2d_grid(obj = dplyr::filter(ebs_layers$survey.area, 
...
write.csv(nbs_grid, row.names = FALSE,
          file = here::here("extrapolation_grids", "nbs_coarse_grid_5nm.csv"))
...
nrow(grid) # check N cells: results in 2250 for 10x10nm resolution

write.csv(grid, row.names = FALSE,
          file = here::here("extrapolation_grids", "bering_coarse_grid_5nm.csv"))