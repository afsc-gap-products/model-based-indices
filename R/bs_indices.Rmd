...
if(species == "kamchatka_flounder"){
  ...
  ggsave(file = here("species_specific_code", "BS", species, phase, "results", 
    ...
    old_i <- read.csv(here("species_specific_code", "BS", species, phase, "results", "Index.csv")) %>%
...
if (species == "pollock") {
  ...
  # Calculate biomass-per-year manually
  bio_pred <- p$data %>%
    group_by(year) %>%
    summarize(biomass = sum(exp(est1 + est2) * area_km2))
  
  # Define a derived quantity as a function of the model's parameters & random effects
  biomass_fn <- function(par, random) {
    # Compute predicted density
    est1 <- obj$report(par, random)$est1
    est2 <- obj$report(par, random)$est2
    
    # Combine with grid info to compute biomass by year
    years <- rep()
    by_year <- splot(data.frame(est = (est1 + est2), 
                                area_km2 = pred_grid$area_km2, 
                                year = pred_grid$year), 
                     grid$year)
    biomass <- sapply(by_year, function(x) sum(exp(x$est) * x$area_km))
    return(biomass)
  }
  
  biom_sd <- RTMB::reportSD(biomass_fn, obj)
  biom_sd$cov
}
