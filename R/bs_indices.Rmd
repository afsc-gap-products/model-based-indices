...
if(species == "pollock"){
  path <- here("species_specific_code", "BS", species, "data")
  
  # Get data file from google drive, if on GCP and file doesn't already exist locally
  if (isTRUE(GCP) && !file.exists(here(path, paste0("VAST_ddc_all_", this_year, ".csv")))) {
    if (!file.exists(here(path, paste0("VAST_ddc_all_", this_year, ".csv")))) {
      library(googledrive)
      ...
      drive_download(file = as_id(file_id),
                     path = here(path, paste0("VAST_ddc_all_", this_year, ".csv")),
                     overwrite = TRUE)
    }
  }
  
  # Read in locally saved data file and reshape
    ...
    if(species == "kamchatka_flounder"){
      #make mesh just for EBS only, need to try out different numbers of knots
        mesh <-  make_mesh(dat, xy_cols = c("X", "Y"), 
                     mesh = fmesher::fm_as_fm(readRDS(file = here("meshes/ebs_vast_mesh_250_knots.RDS"))),
...
grid_ebs <- read.csv(here("extrapolation_grids", "ebs_coarse_grid_5nm.csv"))
grid_nbs <- read.csv(here("extrapolation_grids", "nbs_coarse_grid_5nm.csv"))
grid <- read.csv(here("extrapolation_grids", "bering_coarse_grid_5nm.csv"))
...
if(species == "pacific_cod"){
  title <- bquote('Predicted densities'~(log(n / km^2)))
} else {
  title <- bquote('Predicted densities'~(log(kg / km^2)))
}

sf::sf_use_s2(FALSE)  # turn off spherical geometry
# Crop land area for quicker plotting
bs_sf <- sf::st_crop(rnaturalearth::ne_countries(scale = "medium", 
                                                 returnclass = "sf"), 
                     xmin = -179, xmax = -150, ymin = 50, ymax = 70)

# Re-project spatial density estimate in latitude/longitude
dens_est <- bind_cols(year = p$data$year,
                      est = p$data$est1 + p$data$est2,
                      X = p$data$X,
                      Y = p$data$Y) %>%
  akgfmaps::transform_data_frame_crs(coords = c("X", "Y"), 
                                     in.crs = "+proj=utm +datum=WGS84 +units=km +zone=2",
                                     out.crs = "+proj=longlat +datum=WGS84")

ggplot(dens_est, aes(X, Y, fill = est)) +
  geom_tile(width = 0.456, height = 0.236) +
  geom_sf(data = bs_sf, inherit.aes = FALSE) +
  scale_fill_viridis_c(option = "plasma") +
  facet_wrap(~year) +
  # set limits for the plot extent
  coord_sf(xlim = range(dens_est$X), 
           ylim = range(dens_est$Y), 
           expand = TRUE) +  # ggplot adds a buffer
  scale_x_continuous(breaks = c(-175, -160), labels =c(175, 160)) +
  scale_y_continuous(breaks = c(56, 64), labels = c(56, 64)) +
  ylab("Latitude (\u00B0N)") +
  xlab("Longitude (\u00B0W)") +
  labs(fill = "") +
  ggtitle(title) +
...
ggsave(file = here("species_specific_code", "BS", species, phase, "results",
                   "predictions_map_latlon.png"),
       height = 7.5, width = 6.5, units = c("in"))
```
...
if(species == "kamchatka_flounder"){
  ...
  ggsave(file = here("species_specific_code", "BS", species, phase, "results", 
    ...
    # compare to older models
    # load(here("species_specific_code", "BS", species, "hindcast", "results", "indices_250_knots.RData"))
    # old_i <- bind_rows(ind, ind_ebs, ind_nbs) %>% mutate(index = "mb_old") %>% select(index, region, year, est, lwr, upr)
    # vast
    ...
    #   select(index, year, est, se, region = Stratum) %>% 
    #   filter(year != 2020) %>%
    #   mutate(lwr = exp(log(est) + qnorm(0.025) * se)) %>% 
    #   mutate(upr = exp(log(est) + qnorm(0.975) * se)) %>%
    #   select(-se)
    
    old_i <- readRDS(here("species_specific_code", "BS", species, "archive", this_year, "production", "results", "indices.RDS")) |>
     mutate(index = "mb_old") |> select(index, region, year, est, lwr, upr)
      
    both_i <- bind_rows(new_i, old_i)
    
    ggplot(filter(both_i, region == "Both"), aes(x = year, y = est)) + 
      geom_ribbon(aes(ymin = lwr, ymax = upr, fill = index), alpha = 0.3) +
      geom_point(aes(color = index), size = 1, alpha = 0.8) + 
      geom_line(aes(color = index), alpha = 0.8) + 
    ...
    ggplot(filter(both_i, region == "EBS"), aes(x = year, y = est)) + 
      geom_ribbon(aes(ymin = lwr, ymax = upr, fill = index), alpha = 0.3) +
      geom_point(aes(color = index), size = 1, alpha = 0.8) + 
      geom_line(aes(color = index), alpha = 0.8) + 
      ylim(0, max(both_i$upr)) +
    ...
    ggplot(filter(both_i, region == "NBS"), aes(x = year, y = est)) + 
      geom_ribbon(aes(ymin = lwr, ymax = upr, fill = index), alpha = 0.3) +
      geom_point(aes(color = index), size = 1, alpha = 0.8) + 
      geom_line(aes(color = index), alpha = 0.8) + 
      ylim(0, max(both_i$upr)) +
    ...
    new_i |>
      arrange(desc(region)) |>
        ggplot(aes(x = year, y = est)) + 
          geom_ribbon(aes(ymin = lwr, ymax = upr, fill = region), alpha = 0.5) +
          geom_point(aes(color = region), size = 1, alpha = 0.7) +
          geom_line(aes(color = region), alpha = 0.7) + 
...
if (!file.exists(f5)) {
  ...
  eao_plot <- ggplot(eao_all, aes(x = year, y = est)) +
    geom_point() +
    geom_line(alpha = 0.8) +
