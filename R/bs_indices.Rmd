...
# Convert land data to UTM, then from meters to kilometers
bs_area <- sf::st_crop(world, xmin = -179, xmax = -150, ymin = 50, ymax = 70)
bs_area <- sf::st_transform(bs_area, crs = 32602)
bs_area$geometry <- bs_area$geometry / 1000

ggplot(p$data, aes(X, Y, fill = est1 + est2)) +
  geom_tile(width = 22.224, height = 22.224) +
  geom_sf(data = bs_area, inherit.aes = FALSE) +
  scale_fill_viridis_c(option = "plasma") +
  facet_wrap(~year) +
  # set limits for the plot extent, +/- a buffer
  coord_sf(xlim = c(min(p$data$X) - 100, max(p$data$X) + 100), 
           ylim = c(min(p$data$Y) - 100, max(p$data$Y) + 100), 
           expand = FALSE) +
  scale_x_continuous(breaks = c(250, 750)) +
  scale_y_continuous(breaks = c(6000, 6500, 7000)) +
  ylab("Northings (km)") +
