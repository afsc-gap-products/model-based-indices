...
this_year %in% env_df$YEAR  # Check if this year is included! If FALSE, reinstall coldpool package.
...
ggplot(dens_est %>% filter(year >= 2021), aes(X, Y, fill = est)) +
...
ggsave(file = here("pred_recent.png"), height = 5, width = 6, units = "in", dpi = 300)
ggsave(file = here("species_specific_code", "BS", species, phase, "results",
...
if(species == "kamchatka_flounder"){
  ...
  ggsave(file = here("species_specific_code", "BS", species, phase, "results", 
    ...
    if(phase == "hindcast") {    
      old <- readRDS(here("species_specific_code", "BS", species, "archive", 
                          this_year, "production", "results", "indices.RDS"))
    } else {
      old <- readRDS(here("species_specific_code", "BS", species, 
                          "hindcast", "results", "indices.RDS"))
    }
     
    old_i <- old |> mutate(index = "mb_old") |> select(index, region, 
                                                       year, est, lwr, upr)
