...
# # Percent difference
# comp_diff <- function(new, old, names, save_results = FALSE) {
#   
#   df <- dplyr::bind_rows(new, old) |>
#     filter(year < this_year) |> # drop current year from production output
#     select(-region)
#   
#   #NOT WORKING, START HERE NEXT TIME TO GET DIFFERENCES FOR EACH AGE/YEAR
#   # Get difference between new and old props
#   diff_df <- df |> group_by(year, across(.cols = starts_with("age"))) |>
#     mutate(diff <- diff(starts_with("age")))
#     #select(id, starts_with("age"))
#   
#   if(save_results == TRUE) {  # save to drive, if you want. Check file paths.
#     options(scipen=999)
#     write.csv(diff_df, here(workDir, "results_age", "bridge_props.csv"))
#   }
#   
#   props_plot <- melt(diff_df, id.vars = c("year", "version"), 
#                      variable.name = "Age", value.name = "Proportion") %>%
...
# }
# 
# comp_diff <- comp_diff(new = new_props, old = old_props,
#                               names = c("production", "hindcast"),
#                               save_results = FALSE)
# comp_diff
# 
# # tinyVAST plots save ---------------------------------------------------------
# ggsave(comp_diff, filename = here(workDir, "results_age", "comp_diff.png"),
