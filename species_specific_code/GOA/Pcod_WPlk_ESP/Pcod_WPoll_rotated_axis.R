...
##################################################
which_spp <- c("Pacific cod", "walleye pollock")[2]
spp_code <- spp_names$SPECIES_CODE[which(spp_names$COMMON_NAME == which_spp)]
...
saveRDS(object = data_geostat,
        file = paste0("processed_data/data_geostat_W140_", 
                      gsub(x = which_spp,
                           pattern = " ", 
                           replacement = "_"), 
                      ".RDS"))
...
##################################################
goa_grid <- read.csv("processed_data/GOAThorsonGrid_Less700m.csv") 
...
fit <- fit_model(settings = settings, 
                 working_dir = paste0(getwd(), "/output/", 
                                      which_spp, " rotated axis/"),
...
##################################################
results <- plot( fit, 
                 working_dir = paste0(getwd(), "/output/", which_spp, 
                                      " rotated axis/"),
                 check_residuals = TRUE)

write.csv( results$Range$COG_Table, 
           file = paste0(getwd(), "/output/", which_spp, 
                         " rotated axis/COG.csv"), 
           row.names = FALSE )
...
##################################################
save(fit, file = paste0("output/", which_spp, 
                        " rotated axis/VASTfit.RData"))
