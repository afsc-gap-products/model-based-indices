...
# Simple path for proportions files & remove rowname column generated by R
...
options(scipen=999)
write.csv(check_props_tab, here::here("VAST_results", "bridge_props_2023.csv"))
write.csv(check_props_abs_tab, here::here("VAST_results", "bridge_props_abs_2023.csv"))
...
comp_diff <- ggplot(props_plot %>% filter(Region == "Both" & Year != 2020), 
...
all_props <- rbind.data.frame(new_props_long, old_props_long) %>%
  filter(Year != 2020 & Region == "Both") %>%  # remove 2020
