#' Test of the age composition expansion for BS yfs in tinyVAST. Original
...
speciesName <- paste0("yellowfin_sole_age_", as.character(this_year), "_EBS-NBS")
workDir <- here::here("species_specific_code", "BS", Species)
Data <- readRDS(here::here("species_specific_code", "BS", Species, "production", "data", 
...
family <- list(
  ...
  age_16 = delta_gamma(type = "poisson-link"), 
  age_17 = delta_gamma(type = "poisson-link"), 
  age_18 = delta_gamma(type = "poisson-link"), 
  age_19 = delta_gamma(type = "poisson-link"), 
  age_20 = delta_gamma(type = "poisson-link") 
)
...
# Save fit object
saveRDS(myfit, here(workDir, "hindcast", "results_age", paste0("tinyVAST_fit_", as.character(this_year), "prod_data.RDS")))


# Index expansion -------------------------------------------------------------
# Load fit object if needed
#myfit <- readRDS(here(workDir, "hindcast", "results_age",  paste0("tinyVAST_fit_", as.character(this_year), "prod_data.RDS")))
...
# Get abundance after dropping age:year combinations with all 0s 
...
write.csv(N_ct, here(workDir, "hindcast", "results_age", paste0("tinyVAST_natage_", as.character(this_year), "prod_data.csv")), row.names = FALSE)


# Reformat and calculate proportions ------------------------------------------
rownames(N_ct) <- 1:20
tiny_out <- tibble::rownames_to_column(data.frame(t(N_ct)), "VALUE")
tiny_out <- tiny_out[, c(2:21, 1)]
...
# Save proportions
write.csv(tiny_out, here(workDir, "hindcast", "results_age", paste0("tinyVAST_props_", as.character(this_year), "prod_data.csv")), row.names = FALSE)
